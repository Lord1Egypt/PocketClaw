...
| **Verification APK (PC-DEF-075 canonical bot link)** | `643c2495a529465eff03c12ebe3a17fb82c153affe9fd38e90718bcb88da2f72`, 63,632,335 bytes, `0.2.0+62`, development signer `15cf75f9…`, Dart AOT `5c0f5825…`, Core fingerprint `87c320c1…`, Core BuildTime `2026-09-21T03:22:04+0000`. Source gate 29/29, Go vet/test exit 0, frontend 577 passed, `tsc`/ESLint clean, `flutter analyze` clean. Installed on the Samsung; runtime confirmed up. Archived read-only at `build/forensic/apk-643c2495…/`. **Open chat physically PASSED** on the Samsung (owner, 2026-09-21): the existing Telegram conversation opens, no "Username not found". LOCAL TEST / NON-RELEASABLE — this artifact is evidence, never the published one. |
| Consolidated verification APK (Credentials withdrawal + PC-DEF-050/052/055/061/062/063/066/067/068/069/070/071/072/073/074, superseded) | `8660e72feebab36a7867c2708c02557501ddd20a67422c5576250aee3d816395`, 63,632,711 bytes, `0.2.0+62`, package `com.lord1egypt.pocketclaw`, development signer `15cf75f9…`, Dart AOT `a135bfe6…`, Core fingerprint `9c22cb22…`, Core BuildTime `2026-09-21T02:12:52+0000`. Source gate 29 rows all PASS, artifact gate 33/33, native ELF 188 PASS / 0 FAIL, Zero-Pico 22 entries all in use, Go vet/test exit 0, frontend 566 passed, Flutter 579 passed, Android unit tests 50 passed. Verified inside the packaged Dashboard: `navigation.credentials` 0 occurrences, `/credentials` present only as the `beforeLoad` redirect to `/models`. Archived read-only at `build/forensic/apk-8660e72f…/`. **This is the single build for the remaining physical pass.** LOCAL TEST / NON-RELEASABLE |
...
`release/v0.2.3-fdroid-hardening` (`b0b9745`) merged into `main` as `ee68747`
(tree identical); `develop` fast-forwarded. A canonical build of `ee68747`
reproduced the signed candidate's unsigned APK `e09340e7…` before tagging.
Annotated tag `v0.2.3` at `ee68747`; GitHub release "PocketClaw v0.2.3 —
Golden #4", Latest, not a pre-release, with the APK and `SHA256SUMS.txt`;
downloaded back byte-identical (size, package, 0.2.3/65, minSdk 26,
arm64-v8a, signer). README and `docs/release/published.json` moved in
`920ec4b`.

**Final F-Droid delta:** `canonical_release_build.py build --track-b` of the
tag: the real `fdroid build` retrieved the published APK, "compared built
binary to supplied reference binary successfully" and found the allowed signer
`176dca6b…` — the fourth identical unsigned build (`e09340e7…`, 2,650 s). The
fdroiddata draft (outside this repo) is rendered from `fdroid/metadata.yml.in`
at `ee68747` with `Binaries:` and `AllowedAPKSigningKeys:`; readmeta, lint and
rewritemeta pass, byte-stable. **TRACK B READY. No submission, no merge
request.**

## 2026-09-25 — Golden #4: PocketClaw v0.2.3 accepted (F-Droid hardening)

Branch `release/v0.2.3-fdroid-hardening` from `4d14f37` (main + the Phase C
docs commit only). Phase C's workarounds became upstream fixes:
PC-DEF-091 (NDK path out of payload DWARF, `3e74c09`), the Core epoch
resolver (`bb785ae`: build-input history wins over an inherited
SOURCE_DATE_EPOCH; override `POCKETCLAW_BUILD_EPOCH`), PC-DEF-093 (go1.26.8,
x/crypto v0.56.0 and deps, gh 2.101.0 on go1.27.1, `16b2c6d`), the builder's
gradle fallback (`95c5bef`), Core restaged (`d966ab3`, fingerprint
`cac3e443a7e7e0c509a066e7758de2f0e27207419b894ad9de60ad974fb7dbe0`, BuildTime
2026-09-25T13:31:29+0000), and PC-DEF-092 by the canonical release build
(`4664026`, `7c0980c`). 0.2.3+65 What's New in 12 locales; store text and
README corrected (no "no downloads" claim; Core-scoped memory claim).

**Automated:** source gate 31/0/0 (flutter.suite 630); `flutter analyze`
clean; Android unit 87 (`:app:testDebugUnitTest`); tool tests 181; frontend
585, `tsc`, ESLint clean; Go gofmt/vet (host, android/arm64) clean, 99
packages; govulncheck 1.8.0: Core and gh 0 reachable / 0 imported;
secret-pattern scan v0.2.2..HEAD clean.

**Canonical builds** of `7c0980c` (real `fdroid build --on-server`, buildserver
image `sha256:9cb68105…`, fdroidserver `a35fdfdd`): A and B both
`e09340e7b310a9c95e9ab0e269ad2b6c8968455295a4ab0da0fdc9d48ec0cc61`
(58,434,979 B, 2,214 s and 2,643 s), scanner clean apart from the two
pub-cache globs; every PocketClaw native library equals the staged copy.
Gate on the unsigned APK (repository) 35/0/0, ELF 146/0/0.

**Owner-signed** (apksigner v2 only, alignment preserved, helper outside the
repo): `PocketClaw-v0.2.3-arm64-v8a.apk`
`778882828e2f818aac31ed1f58dd3b2e8530828970b535ce72564f5ed2857599`,
58,441,695 B, signer `176dca6b…`. `verify_apks` against build B: MATCH; B +
signature byte-identical. `release_gate.py --full --release-class production
--artifact-class public-release`: 65/0/0; native ELF 152/0/0 with the private
manifest bound to this APK.

**Physical (SM-A165F, Android 16, Windows adb):** `install -r` over Golden #3:
Success, installed base.apk hashes to `77888282…`, firstInstallTime
2026-09-21 08:07:40 unchanged, 48-file workspace identical. On launch Core
auto-started (preference kept), Public Mode ON kept (LAN `/` 302 to
`/launcher-login`, APIs 401, gateway refused from LAN), Arabic kept, Telegram
long-poll to `api.telegram.org` resumed, default model deepseek-v4.1-flash kept.
What's New shows 0.2.3 open with 0.2.2/0.2.1/0.2.0 collapsed (0.2.2 expands
with its notes, RTL); About 0.2.3 / runtime 0.3.1. Short chat answered
("Golden OK"); a tool task ran the bundled gh and reported 2.101.0. All seven
tools run from nativeLibraryDir (git 2.51.0, gh 2.101.0, curl 8.11.1, rg
14.1.1, sqlite3 3.50.4, jq 1.7.1, python 3.14.7 with Core's environment). No
PocketClaw crash-class logcat entry. Fresh throwaway Core: no external
connection in 180 s. Core idle about 18 MB PSS (32.9 MB right after the tool
task). Reboot not repeated: no service or boot code changed. Owner phone
settings restored. Found, not fixed: the four bottom-navigation icons have no
...
**PC-DEF-075.** The final release pass was halted by a physical defect: the
Telegram card showed Connected and the bot chat worked, but "Open chat" got
**"Username not found"** — so only the generated destination was wrong.

The link was built by raw interpolation, `https://t.me/<value>`, in three
places, from a username the onboarding service supplies. Telegram's canonical
bot link carries no `@` in the path. The codebase disagreed with itself about
the shape it held — one doc comment promised an `@username`, two formatters
added the `@` themselves — and Core already stripped `@` internally before
comparing usernames, a defence the link path never had.

A second, independent cause produces the identical symptom: the Android host
caches a username written only by the native pairing launcher, so a bot paired
any other way points at a bot that may no longer exist. Canonicalisation alone
does not fix that, so both halves are closed: one canonicaliser per language,
and `GET /api/telegram/identity` as the authority, reading `getMe` against the
committed credential and canonicalising in Core.

**Release state: the v0.2.0 final audit passed in full at `010a9d0`** (Go,
frontend, Flutter, Android, Zero-Pico, source gate 29/29) and is re-confirmed
here. The Open chat retest **passed physically** on 2026-09-21, which cleared
the last release blocker. Production signing is the owner's step by design: the
keystore passwords are owner-held and outside this repository, and they must
never enter model or chat context.
