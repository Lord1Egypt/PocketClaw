...
func TestHandleIncoming_DoesNotConsumeGenericCommandsLocally(t *testing.T) {
	...
	// selfNumber and allowList are seeded because this channel reads only the
	// configured Self-Chat and denies an unconfigured sender. The subject here
	// is command handling, so the message is a self-chat one and the command is
	// what is under test.
