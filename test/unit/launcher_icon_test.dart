...
void main() {
  ...
  group('no launcher resource carries the old artwork', () {
    ...
    test('the glossy 3D mark is gone from every surface', () {
      // It survived the launcher move as the pre-Android-12 splash and as the
      // README picture. Both are generated from the APERTURE strokes now, so
      // no surface can show a mark the launcher does not (PC-DEF-081).
      expect(File('assets/branding/pocketclaw-mark.png').existsSync(), isFalse);
      expect(File('$res/drawable/pocketclaw_mark.png').existsSync(), isFalse);
      expect(
        File('$res/mipmap-anydpi-v26/ic_launcher.xml').readAsStringSync(),
    ...
    test('the splash and the status-bar icon are derived, not drawn', () {
      final generator =
          File('tool/generate_android_launcher_icons.py').readAsStringSync();
      expect(generator, contains('drawable-nodpi/pocketclaw_mark.png'));
      expect(generator, contains('drawable/ic_stat_pocketclaw.xml'));
      for (final splash in [
        '$res/drawable/launch_background.xml',
        '$res/drawable-v21/launch_background.xml',
      ]) {
        expect(File(splash).readAsStringSync(),
            contains('@drawable/pocketclaw_mark'));
      }
      expect(
        File('$res/drawable/ic_stat_pocketclaw.xml').readAsStringSync(),
        contains('Generated by tool/generate_android_launcher_icons.py'),
      );
    });
  });
  ...
  group('the generator cannot revert the launcher', () {
    ...
    // flutter_launcher_icons is not a dependency at all any more. Its Android
    // generation was already off; the only targets it still served were the
    // Windows and macOS icons of a desktop build PocketClaw does not have.
    test('no icon generator but ours is configured', () {
      expect(pubspec, isNot(contains('flutter_launcher_icons:')));
      expect(pubspec, isNot(contains('image_path:')));
    });
    ...
    test('holds no pointer back to the glossy 3D mark', () {
      ...
      expect(pubspec, isNot(contains('pocketclaw-mark.png')));
    });
